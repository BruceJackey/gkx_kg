...
import { Search, Zap, Clock, CheckCircle, Copy } from 'lucide-react';
import type { NodeSimilarityFocus } from '../../data/auditPageMap';
...
interface ApiPlaygroundProps {
  ...
  onRun: () => void;
  results: { node: typeof NODES[0]; score: number }[];
  apiCopied: boolean;
  onCopy: () => void;
  badge?: string;
}
...
function ApiPlayground({
  ...
  const algoOptions = ALGOS.filter(a => allowedAlgos.includes(a.id));
  ...
  return (
    <div className="border-2 border-blue-200 rounded-xl overflow-hidden">
      <div className="bg-blue-50 border-b border-blue-200 px-5 py-3 flex items-center justify-between">
        ...
        <span className="text-xs text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full">{badge}</span>
      </div>
      <div className="p-5 space-y-4">
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <select
              value={selectedAlgo}
              onChange={e => onAlgoChange(e.target.value as AlgoId)}
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            >
              {apiCopied
                ? <><CheckCircle className="w-3.5 h-3.5 text-green-600" /><span className="text-green-600">已复制</span></>
...
interface NodeSimilarityDemoProps {
  initialSection?: NodeSimilarityFocus;
}

export function NodeSimilarityDemo({ initialSection }: NodeSimilarityDemoProps) {
  const sectionFromFocus = (focus?: NodeSimilarityFocus) => {
    if (focus === 'embedding') return 'embedding' as const;
    if (focus === 'api') return 'api' as const;
    return 'path' as const;
  };
  const algoFromFocus = (focus?: NodeSimilarityFocus): AlgoId => {
    if (focus === 'embedding') return 'cosine';
    if (focus === 'api') return 'ppr';
    return 'ppr';
  };

  const [activeSection, setActiveSection] = useState<'path' | 'embedding' | 'api'>(sectionFromFocus(initialSection));
  const [selectedAlgo, setSelectedAlgo] = useState<AlgoId>(algoFromFocus(initialSection));
  ...
  useEffect(() => {
    ...
    setActiveSection(sectionFromFocus(initialSection));
    setSelectedAlgo(algoFromFocus(initialSection));
    setLatencyMs(null);
  }, [initialSection]);
  ...
  const runQuery = () => {
    ...
    setRunning(true);
    setLatencyMs(null);
    const fakeLatency = selectedAlgo === 'cosine'
      ? Math.round(Math.random() * 0.8 + 0.2)
      : selectedAlgo === 'ppr'
  ...
  const copyApi = () => {
    navigator.clipboard.writeText(API_EXAMPLE(selectedAlgo, sourceNode, topK));
    setApiCopied(true);
  ...
  const playgroundProps = {
    selectedAlgo,
    onAlgoChange: (id: AlgoId) => { setSelectedAlgo(id); setLatencyMs(null); },
    sourceNode,
    onSourceChange: (id: string) => { setSourceNode(id); setLatencyMs(null); },
    topK,
  ...
  const SECTIONS = [
    { id: 'path' as const, label: '① 路径算法 API' },
    { id: 'embedding' as const, label: '② 嵌入向量 API' },
    { id: 'api' as const, label: '③ 统一实时接口' },
  ];
  ...
  return (
    <div className="space-y-5">
        ...
        {SECTIONS.map(s => (
          <button
            ...
            onClick={() => {
              ...
              setLatencyMs(null);
              if (s.id === 'embedding') setSelectedAlgo('cosine');
              if (s.id === 'path' && selectedAlgo === 'cosine') setSelectedAlgo('ppr');
            }}
      ...
      {activeSection === 'path' && (
        <div className="space-y-5">
          <div>
            <h3 className="text-base font-semibold text-gray-900">基于路径的相似度算法 API</h3>
            <p className="text-sm text-gray-500 mt-0.5">
              通过 <code className="text-xs bg-gray-100 px-1 rounded">POST /api/node-similarity</code> 调用 Personalized PageRank、SimRank，无需训练，直接利用图拓扑结构计算节点相关性
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {ALGOS.filter(a => a.type === 'path').map(a => (
              <button
                key={a.id}
                onClick={() => { setSelectedAlgo(a.id); setLatencyMs(null); }}
          ...
          <ApiPlayground {...playgroundProps} allowedAlgos={['ppr', 'simrank']} badge="路径算法 API" />
          <div className="border border-gray-200 rounded-xl p-4">
            <p className="text-xs font-semibold text-gray-600 mb-2">
              {source.label} 的直接邻居（共 {ADJ[sourceNode].length} 个）— 路径算法依赖图结构
            </p>
            <div className="flex flex-wrap gap-2">
              {ADJ[sourceNode].map(nid => {
                ...
                return (
                  <button
                    key={nid}
                    onClick={() => { setSourceNode(nid); setLatencyMs(null); }}
      ...
      {activeSection === 'embedding' && (
        <div className="space-y-5">
          <div>
            <h3 className="text-base font-semibold text-gray-900">基于嵌入向量的相似度算法 API</h3>
            <p className="text-sm text-gray-500 mt-0.5">
              通过 <code className="text-xs bg-gray-100 px-1 rounded">POST /api/node-similarity</code> 传入 <code className="text-xs bg-gray-100 px-1 rounded">algorithm: cosine</code>，使用已训练实体向量计算余弦相似度
            </p>
          </div>
          ...
          <ApiPlayground {...playgroundProps} allowedAlgos={['cosine']} badge="嵌入向量 API" />
          <div className="border border-gray-200 rounded-xl p-5 space-y-3">
            <p className="text-sm font-semibold text-gray-700">向量计算原理（4 维示例）</p>
            <div className="bg-purple-50 border border-purple-200 rounded-lg px-4 py-3 text-sm">
              ...
              <span className="text-gray-600"> = (h·t) / (‖h‖·‖t‖) = </span>
      ...
      {activeSection === 'api' && (
        <div className="space-y-5">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">PPR、SimRank、余弦相似度三类算法统一封装为单一低延迟 REST API，支持 Top-K 返回、类型过滤与分数阈值</p>
          </div>
          <ApiPlayground {...playgroundProps} allowedAlgos={['ppr', 'simrank', 'cosine']} badge="统一实时接口" />
